...
def advanced_feature_engineering(X_train, missing_fill_strategy):
    ...
    # 1. Financial Ratios
    if (
        "Annual_Income" in X_enhanced.columns
        and "Outstanding_Debt" in X_enhanced.columns
    ):
        # Debt-to-Income Ratio
        X_enhanced["Debt_to_Income_Ratio"] = X_enhanced["Outstanding_Debt"] / (
            X_enhanced["Annual_Income"] + 1
        )
        print("✓ Created Debt_to_Income_Ratio")

    if (
        "Monthly_Inhand_Salary" in X_enhanced.columns
        and "Total_EMI_per_month" in X_enhanced.columns
    ):
        # EMI to Salary Ratio
        X_enhanced["EMI_to_Salary_Ratio"] = X_enhanced["Total_EMI_per_month"] / (
            X_enhanced["Monthly_Inhand_Salary"] + 1
        )
        print("✓ Created EMI_to_Salary_Ratio")

    # 2. Credit Behavior Features
    if (
        "Num_Credit_Card" in X_enhanced.columns
        and "Num_Bank_Accounts" in X_enhanced.columns
    ):
        # Credit Cards per Bank Account
        X_enhanced["Cards_per_Account"] = X_enhanced["Num_Credit_Card"] / (
            X_enhanced["Num_Bank_Accounts"] + 1
        )
        print("✓ Created Cards_per_Account")

    if (
        "Num_of_Delayed_Payment" in X_enhanced.columns
        and "Num_of_Loan" in X_enhanced.columns
    ):
        # Delayed Payment Rate
        X_enhanced["Delayed_Payment_Rate"] = X_enhanced["Num_of_Delayed_Payment"] / (
            X_enhanced["Num_of_Loan"] + 1
        )
        print("✓ Created Delayed_Payment_Rate")

    # 3. Investment vs Income
    if (
        "Amount_invested_monthly" in X_enhanced.columns
        and "Monthly_Inhand_Salary" in X_enhanced.columns
    ):
        X_enhanced["Investment_Rate"] = X_enhanced["Amount_invested_monthly"] / (
            X_enhanced["Monthly_Inhand_Salary"] + 1
        )
        print("✓ Created Investment_Rate")

    # 4. Credit Health Score (composite feature)
    credit_health_components = []

    if "Credit_Utilization_Ratio" in X_enhanced.columns:
        # Lower utilization is better
        credit_health_components.append(
            1 - X_enhanced["Credit_Utilization_Ratio"].clip(0, 1)
        )

    if "Delayed_Payment_Rate" in X_enhanced.columns:
        # Lower delay rate is better
        credit_health_components.append(
            1 - X_enhanced["Delayed_Payment_Rate"].clip(0, 1)
        )

    if "Credit_History_Age" in X_enhanced.columns:
        # Longer history is better (normalize to 0-1)
        max_age = X_enhanced["Credit_History_Age"].max()
        if max_age > 0:
            credit_health_components.append(X_enhanced["Credit_History_Age"] / max_age)

    if credit_health_components:
        X_enhanced["Credit_Health_Score"] = np.mean(credit_health_components, axis=0)
        print("✓ Created Credit_Health_Score")

    # 5. Binning for skewed features
    skewed_features = ["Annual_Income", "Outstanding_Debt", "Monthly_Inhand_Salary"]

    for feature in skewed_features:
        if feature in X_enhanced.columns:
            # Create quantile-based bins
            X_enhanced[f"{feature}_Bin"] = pd.qcut(
                X_enhanced[feature],
                q=5,
            ...
            # Convert to numeric
            X_enhanced[f"{feature}_Bin"] = pd.Categorical(
                X_enhanced[f"{feature}_Bin"]
            ).codes
            print(f"✓ Created {feature}_Bin")

    # 6. Interaction features for important combinations
    interactions = [
    ...
    print(f"Feature engineering completed! New shape: {X_enhanced.shape}")
    print(f"Added {X_enhanced.shape[1] - X_train.shape[1]} new features")
...
def save_model_and_pipeline(
    ...
    pipeline_path = os.path.join(models_dir, "xgboost_preprocessing_pipeline.pkl")
    ...
    info_path = os.path.join(models_dir, "xgboost_model_info.pkl")
